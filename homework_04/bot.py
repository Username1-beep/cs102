...
telebot.apihelper.proxy = {'https': 'socks5://ItmoProxy:GreatBots@proxy.prokhn.ru:1080'}
    ...
    while not found:
        ...
        if schedule is not None:
            ...
            if this_day:
                ...
                for i in range(len(times_list)):
                    lesson_start_time = time.strptime(times_list[i].split('-')[0], '%H:%M')
            ...
            days_to_check -= 1
            if days_to_check == 0:
                bot.send_message(message.chat.id, 'Не найдено ближайшей пары для указанной группы')
                return
...
def get_tommorow(message):
    ...
    message.text = f'/{WEEKDAYS[day]} {group}'
